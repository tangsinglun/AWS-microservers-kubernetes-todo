...
//         const decodeToken = verify(token, cert, { algorithms: ['RS256'] }, (err, decoded) => {
//           if (err) {
//             return res.status(500).send({ auth: false, message: 'Failed to authenticate.' });
//           }
//         });

//         return res.status(200).send({ decode: decodeToken});
//         // next();
...
router.post('/', 
  ...
  try {
        ...
        // Generate JWT
        // const jwt = generateJWT(newitem);
          
        res.status(201).send(item);
        // res.status(200).send({ auth: true, token: jwt, item: item});
  }                                        
  catch(error: any){
      throw new Error(error.message);
  }
...
router.patch('/:todoId', 
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            // Generate JWT
            // const jwt = generateJWT(item);
            
            res.status(201).send();
            // res.status(201).send({ auth: true, token: jwt, item: item});
        }
        catch(error: any) {
            throw new Error(error.message);
        } 
